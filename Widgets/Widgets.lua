...
function addon:CreateMovableFrame(title, name, width, height, frameStrata, frameLevel, notUserPlaced)
    ...
    header.closeBtn = addon:CreateButton(header, "×", "red", {20, 20}, false, false, "ABSTRACT_FONT_SPECIAL", "ABSTRACT_FONT_SPECIAL")
...
-----------------------------------------------------------------------------------
function addon:CreateScrollFrame(parent, top, bottom, color, border, noScrollBar)
    ...
    if not noScrollBar then
        -- scrollbar
        scrollbar = CreateFrame("Frame", nil, scrollFrame, "BackdropTemplate")
        ...
        -- scrollbar thumb
        scrollThumb = CreateFrame("Frame", nil, scrollbar, "BackdropTemplate")
...
local function CreateBar(parent)
    ...
    b.rightText = b:CreateFontString(nil, "OVERLAY", font_name)
    b.rightText:SetPoint("RIGHT")
    function b:SetRightText(text)
        b.rightText:SetText(text)
    end

    return b
end

function addon:CreateScrollChart(parent)
